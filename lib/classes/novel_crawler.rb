...
class NovelCrawler
  ...
  def crawl_articles novel_id
    ...
    elsif(@page_url.index('aka99.com'))
    ...
    elsif(@page_url.index('xs555.com'))
    ...
    elsif(@page_url.index('ranwenba'))
      url = @page_url
      nodes = @page_html.css(".book_booklist .contentandborder")
      nodes = nodes[0].css("a")
      nodes.each do |node|
          article = Article.find_by_link(url + node[:href])
          next if (article != nil && article.text != nil)
    ...
    elsif (@page_url.index('dushiwenxue'))
      url = @page_url.sub("index.html","")
      nodes = @page_html.css(".ccss a")
      nodes.each do |node|
        article = Article.find_by_link(url + node[:href])
    ...
    elsif (@page_url.index('shubao2.com'))
    ...
    elsif(@page_url.index('read.qidian'))
      novel = Novel.select("id,num,name").find(novel_id)
      subject = novel.name
    ...
    elsif(@page_url.index('ranwen.cc'))
      novel = Novel.select("id,num,name").find(novel_id)
      subject = novel.name
  ...
  def crawl_text_onther_site article
    if(@page_url.index("yantengzw"))
      ...
      article.text = ZhConv.convert("zh-tw", text.strip)
      article.save
    elsif (@page_url.index('txtbbs.com'))
      node = @page_html.css("#content")
      node.css("img").remove
      text = change_node_br_to_newline(node).strip
      article.text = ZhConv.convert("zh-tw", text.strip)
      ...
      article.text = ZhConv.convert("zh-tw", text.strip)
      article.save
    elsif(@page_url.index('www.ttshu.com'))
