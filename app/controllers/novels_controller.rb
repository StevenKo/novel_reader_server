class NovelsController < ApplicationController
  ...
  def show
    ...
    @articles = Article.select("articles.id,title,subject,num,is_show, novel_id").where("novel_id = #{params[:id]} and is_show = true").order(sort_column + " " + sort_direction).paginate(:page => params[:page], :per_page => 5).order("num ASC")
